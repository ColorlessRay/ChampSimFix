	...
	class RoRaCoBaBgCh final : public IAddrMapper, public Implementation {
  	...
  	public:
  		//basic set up
    		...
    		// To print onto seperate file
    		std::vector<double> power_com_final;
...
//------------------------------------------------------------------------------------------------------------------------
    		...
    		void apply(Request& req) override {
      			...
      			if (!previous.empty() && previous.size() == og.size()) {
        			...
        			double power_com;
				power_com = static_cast<double>(total_differing_bits) / total_bits_compared * 100;
				std::cout << std::fixed << std::setprecision(4);
				//std::cout << power_com << std::endl;
	
				std::cout << "Total power consumption: " << power_com<< "%" << "\n";
	
				// Add power consumption to the final list for writing to file
      				power_com_final.push_back(power_com);
      			}
      			...
      			*/
      			...
      			writePowerConsumptionRatesToFile("RoRaCoBaBgCh_Final_output_power_com.txt");
    		...
    		void writePowerConsumptionRatesToFile(const std::string& filename) const {
    			...
    			if (outFile.is_open()) {
      				for (size_t i = 0; i < power_com_final.size(); ++i) {
        				outFile << "Rate for instruction " << i << ": " << power_com_final[i] << "%" << std::endl;
      				}
	...
	public:
  		...
  		void apply(Request& req) override {
    			...
    			if (!previous.empty() && previous.size() == og.size()) {
      				...
      				double power_com = static_cast<double>(total_differing_bits) / total_bits_compared * 100;
      				std::cout << "Total power consumption: " << power_com << "%" << "\n";
    			...
    			previous = og;
    			writePowerConsumptionRatesToFile("Rubix_S_Final_output_power.txt");
  		}
